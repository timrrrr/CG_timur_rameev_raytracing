...
{
	float3 eye = get_position();
	float3 at = eye + get_direction();
	float3 up = get_up();

	float3 z_axis = normalize(eye - at);
...
{
	return float4x4{
			{1 / (tan(angle_of_view / 2) * aspect_ratio), 0, 0, 0},
			{0, 1 / tan(angle_of_view / 2), 0, 0},
...
{
	return float3{cos(phi) * sin(theta), sin(phi), -cos(theta) * cos(phi)};
}
